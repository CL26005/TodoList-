...
public class SimpleTaskManager {
    ...
    public class TaskIdManager {
        ...
        private static AtomicInteger counter = new AtomicInteger(loadCounter());
    ...
    static void viewTasks() {
        ...
        if (tasks.isEmpty()) {
            ...
            for (int i = 0; i < tasks.size(); i++) {
                String[] task = tasks.get(i);
                String dependencies = task[8].replace("|", ", ").trim(); // Convert "|" to ", " and trim any extra spaces
    
                // Filter out deleted tasks from dependencies
                StringBuilder validDependencies = new StringBuilder();
                for (String dependencyId : dependencies.split(", ")) {
                    if (getTaskByID(dependencyId) != null) { // Check if the dependency task still exists
                        if (validDependencies.length() > 0) {
                            validDependencies.append(", ");
                        }
                        validDependencies.append(dependencyId);
                    }
                }
    
                if (validDependencies.length() == 0) {
                    dependencies = "None"; // If no valid dependencies, display "None"
                } else {
                    dependencies = validDependencies.toString(); // Update dependencies with valid ones
                }
    
                System.out.printf("%3d.  Task ID: %-3s %-35s [ Description: %-15s Due: %-12s Category: %-10s Priority: %-7s]  (%-10s)    Dependencies: %s\n",
                        (i + 1), task[7], task[0], task[1], task[2], task[3], task[4], task[5], dependencies);
            }
        }
    }
    ...
    static void taskDependencies() {
        ...
        while (true) {
            ...
            while (!found) {
                ...
                if (getTaskByID(dependentTaskNumber) != null) {
                    found = true;
                } else {
            ...
            while (!found) {
                ...
                if (getTaskByID(precedingTaskNumber) != null) {
                    found = true;
                } else {
                    System.out.println("Invalid task ID! Please try again.");
                }
            ...
            if (isCyclic(dependentTaskNumber, precedingTaskNumber)) {
                System.out.println("Error: This dependency creates a cycle. Please try again.");
                continue;
            }

            String[] task = getTaskByID(dependentTaskNumber);
            HashSet<String> dependencies = new HashSet<>(Arrays.asList(task[8].split("\\|")));
            dependencies.add(precedingTaskNumber);
            task[8] = String.join("|", dependencies);

            System.out.println("Task \"" + task[0] + "\" now depends on \"" + getTaskByID(precedingTaskNumber)[0] + "\".");
            break;
        }
    }

    private static boolean isCyclic(String dependentTaskNumber, String precedingTaskNumber) {
        ArrayList<String> visited = new ArrayList<>();
        return hasCycle(precedingTaskNumber, dependentTaskNumber, visited);
    }

    private static boolean hasCycle(String currentTaskNumber, String taskToCheckNumber, ArrayList<String> visited) {
        if (currentTaskNumber.equals(taskToCheckNumber)) {
            return true;
        }
        ...
        String[] currentTask = getTaskByID(currentTaskNumber);
        if (currentTask == null || currentTask.length < 2) {
            visited.remove(currentTaskNumber);
            return false;
        }
        ...
        for (String dependency : dependencies) {
            if (hasCycle(dependency, taskToCheckNumber, visited)) {
                return true;
            }
        }
    ...
    static void markTask(String[] task) {
        for (String id : task[8].split("\\|")) {
            if (id.isEmpty()) continue;
            String[] dependedTask = getTaskByID(id);
            if (dependedTask == null)
                continue;
            if (!dependedTask[5].equalsIgnoreCase("Complete")) {
                System.out.println("\nWarning: Task \"" + task[0] +
                        "\" cannot be marked as complete because it depends on \"" +
                        dependedTask[0] + "\". Please complete the preceding task first.");
                return;
            }
        }
        task[5] = "Complete";
